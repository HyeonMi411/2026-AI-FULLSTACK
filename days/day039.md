...
1. 처리해줄경로              :  result.jsp          #1      action="[#1result.jsp]"
2. 주소표시창줄 노출여부 o                           #2     method="[#2]"
3. 쿼리스트링으로 해서 넘길때 이름  userage             #3      name="[#3userage]"

     <form action="[#1result.jsp]" method="[#2]" onsubmit="return check()">
      <div class="my-2">
...
[#4int userage = Integer.parseInt( request.getParameter("userage")); ]
//2.  만약 19세 미만이라면 c.jsp 파일 넘기기 (sendRedirect)  [#5response.sendRedirect("c.jsp");]
