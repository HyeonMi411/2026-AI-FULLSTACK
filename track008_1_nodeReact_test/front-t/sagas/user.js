...
export function searchUsersApi(keyword){
    return client.get(`/user/search-user?keyword=${keyword}`);  // 닉네임 부분 검색 API 호출   http://localhost:3065/user/search-user?keyword=xxx
}
export function* searchUsers(action){
    try{
        const result = yield call(  searchUsersApi ,  action.data );  // API 호출 - (비동기 → call)  action.data : keyword
        const users = result.data.map((u) => ({
            id: u.APP_USER_ID,
            email: u.EMAIL,
            nickname: u.NICKNAME,
        }));
        yield put(  { type: SEARCH_USERS_SUCCESS , data: users } );  // 성공 액션 dispatch
    }catch(err){
        yield put(  { type: SEARCH_USERS_FAILURE , error: err.response?.data || err.message} );  // 실패 액션 dispatch
    }
}
function * watchSearchUsers(){
    yield takeLatest(SEARCH_USERS_REQUEST,  searchUsers);
    // 1. SEARCH_USERS_REQUEST 액션발생 , takeLatest (여러번요청 → 가장마지막요청처리)
}
...
export default function* userSaga(){
    yield all([
        ...
        fork(watchCheckNickname), // 닉네임 중복 확인 saga 실행
        fork(watchSearchUsers),   // 닉네임 부분 검색 saga 실행
    ]);
}
