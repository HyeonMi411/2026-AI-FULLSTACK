  ...
  <form action="${pageContext.request.contextPath}/users/join" method="post" onsubmit="return checkForm()">
	...
	<script>
	  	window.addEventListener("load" , function(){
	  		...
	  		let ttarget = document.querySelector(".tnickname");
	  		...
	  		nickname.addEventListener("keyup" , function(  e ){ //console.log( e.target.value );
	  			...
	  			if(value !== ""){ //빈칸이 아니면 서버에 요청
	  				...
	  				.then( data=>{  
	  					if(data.exists){
	  						ttarget.textContent="이미 사용중인 닉네임입니다.";
	  						ttarget.className ="my-3  alert  alert-danger  target";
	  					}else{
	  						ttarget.textContent="사용가능한 닉네임입니다.";
	  						ttarget.className ="my-3  alert  alert-success  target";
	  					}
	  				}).catch(err =>{  
	  					ttarget.textContent="서버오류입니다.";
	  					ttarget.className ="my-3  alert  alert-info  target";
	  				});
	  			} 
	  			else{//빈칸이면
	  				ttarget.textContent="닉네임입니다 중복검사는 필수입니다.";
	  				ttarget.className ="my-3  alert  alert-warning  target";
	  			}  //빈칸이면 메시지 유지
	  		}); 
	  	});
	</script>        
