...
<body>
   ...
   <div class="container card my-5">
     ...
     <!-- 			 -->
      ...
      <script>
      function checkbox() {    	  
    	  let username = document.getElementById("username");		//1. 아이디값 이용해서  query 대상찾아오기    	  
    	  if(username.value.trim() == ""){ alert("검색어를 입력하세요."); username.focus(); return false; }    	  
    	  return true;
	  }          
      </script> 
      ...
      2. jsp에서 다른페이지로 이동
      		(1) sendRedirect	  - 브라우저 url을 변경o, rewuest/response객체가 유지안됨.
      		(2) RequestDispatcher - 위임(요청전달)
      							    브라우저 url을 변경x, rewuest/response객체가 유지 됨.      							          
      </pre>
      <%
      //ver-1 알림창 처리안됨.
      ...
      <form action="jsp013_4.jsp" method="get" onsubmit="return check()">      
		...
		<script>
	     	function order(){
	     		...
	     		if(userage.value.trim() ==""){ alert("나이를 입력하세요"); oname.focus();	return false; }	     		      		
	     		return true;      			
	     	}
		</script>
      ...
      <script>
      function check() {    	  
    	  let userage = document.getElementById("userage");		//1. 아이디값 이용해서  query 대상찾아오기
    	  if(userage.value.trim() == ""){//2. query.value 문자열찾아와서  trim() 앞뒤 공백빼고  
    		  alert("검색어를 입력하세요.");
    		  userage.focus();  //커서
    	  	  return false;//3. 전송여부		    		  
    	  }
...
1. application(웹애플리케이션이 실행되고 있는동안 , 서버가 꺼질때까지 유지) 
		> session (특정브라우저와 연결된 세션, ㅂ므라우저 종료시 소멸) 
			> request(요청: from, a 요청) 
				> page (해당jsp 페이지에서만) 
