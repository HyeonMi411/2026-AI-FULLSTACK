...
-- +----------+------+-------+
-- 1 row in set (0.00 sec)
select    123.4578 , ceil(123.4) `CEIL` , floor(123.4) `FLOOR`
from  dual;
...
-- mysql> -- 2. upper/lower 
select 'ABC' `DEFAULT` , upper('abc') `s1` , lower('ABC') `s2` 
from dual;
-- +---------+------+------+
-- | DEFAULT | s1   | s2   |
-- +---------+------+------+
...
--  length(갯수)		left//right(문자열, 몇개) , replace(문자열, 찾아서 , 바꾸기) , concat(문자열, 문자열,문자열)
--  upper(대문자)/lower(소문자) fn_select_userinfo2
--  6. 이름을 소문자로 조회하시오.
...
create table select_userinfo2  select * from select_userinfo;
alter table select_userinfo2   modify no int not null  auto_increment primary key;
alter table select_userinfo2 add email varchar(100) ;
select * from select_userinfo2;
desc select_userinfo2;

insert into select_userinfo2 (no , name , age , email) values (1 , 'aaaa'   ,  11 , 'aaa@gmail.com');
insert into select_userinfo2 (no , name , age , email) values (2 , 'bbaab'  ,  22 , 'bbb@gmail.com');
insert into select_userinfo2 (no , name , age , email) values (3 , 'ccaac'  ,  33 , 'ccc@gmail.com');
insert into select_userinfo2 (no , name , age , email) values (4 , 'daadd'  ,  44 , 'abc@gmail.com');
insert into select_userinfo2 (no , name , age , email) values (5 , 'abc'    ,  55 , 'abc@gmail.com');
insert into select_userinfo2 (no , name , age , email) values (6 , 'baaca'  ,  66 , 'bca@gmail.com');
