...
-- | sns   | char(2)     | YES  |     | y       |                |
-- +-------+-------------+------+-----+---------+----------------+
use mbasic;     show tables;     desc group_userinfo;     drop table group_userinfo;
...
insert into group_userinfo (no, name, age, sex, kor, eng, math, ban, sns) values ( 1 , 'first'  , 11 , NULL , 100 , 100 , 99 , 'A' , 'n' );
...
-- 10:42:14	select ban,   count(*) from   group_userinfo LIMIT 0, 1000	Error Code: 1140. In aggregated query without GROUP BY, expression #1 of SELECT list contains nonaggregated column 'mbasic.group_userinfo.ban'; this is incompatible with sql_mode=only_full_group_by	0.000 sec
...
select   ban , count(case when age < 20 then 1 end) as `미성년자수`
from     group_userinfo
group by ban;
...
select ban , count(case when sns = 'n' then 1 end) as `sns수신거부수`
from group_userinfo
group by ban;
...
select   ban  , sum(avg(kor+eng+math)) `평균총점` 
from     group_userinfo;
where    ban
group by avg(kor+eng+math) >= 80;
...
-- select    count(deptno) `부서30 직원수` from emp group by  deptno having deptno=30;
-- +---------------+
-- | 부서30 직원수 |
-- +---------------+
-- |             6 |
...
-- +------------------------+
-- 1 row in set (0.00 sec)
-- select    count(comm) `추가수당을받는 사원수` from      emp  group by  comm  having    comm is not null;
-- --------------------------------------------------------
-- -- 연습문제-8
...
select max(sal) `부서10 최대급여` from emp  where  deptno=10;								-- 집계전 필터링
