...
select   *
from     emp
where    deptno in (20,30) ;		-- #		-- 부서번호가 20 또는 30   (in 사용해보기)
...
select *
from   emp
where  sal = ANY(  select   deptno, max(sal) `부서최고급여`
				             from     emp
                             group by deptno );
...
-- any 1개라도 맞으면 참    sal <  amy(1,2,3)  최대보다 작다  ######
-- all 모두 맞아야 참      sal  <  all (1,2,3)  최소보다 작다

--     ANY를 이용하여 30번 부서의 최고 급여
select   *
from     emp  
...
-- step1)
select sal from emp where deptno=30 order by sal asc;  -- 950,1250,1500,1600,2850
-- step2) in =any =some
...
-- --------------------------------------------------------
-- -- 연습문제-16
-- -- emp 테이블에서  부서번호가 10인 부서명이 존재하다면 그부서의 사원데이터를 조회하시요.
...
-- --------------------------------------------------------
-- -- 연습문제-17
-- -- emp 테이블에서  부서번호가 50인 부서명이 존재하다면 그부서의 사원데이터를 조회하시요.
-- -- ※ EXISTS 서브쿼리에 결과값이 하나이상 존재하면 조건식이 모두 TRUE , 존재하지 않으면 FALSE 
-- 선택된 행 없음
...
-- -- FORM 절에서 부서번호가 10인 사원정보를 구하는 인라인뷰(FROM절에 사용하는 서브쿼리)를 이용하세요.
-- +-------+--------+--------+------------+----------+
...
-- |  7698 | BLAKE  | SALES      | 1981-15-01 | CHICAGO  | 2850 |     4 |
...
-- -- 연습문제-24
-- -- emp ,SALGRADE테이블에서 
-- -- 직책이 SALESMAN 인 사람들의 최고 급여보다 높은 급여를 받는 사원들의 정보를 다음과 같이 조회하시요. 
-- -- 다중행 함수 사용하지 않는 방법
-- +-------+--------+------+-------+
